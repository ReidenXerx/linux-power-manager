...
remove_configs() {
    ...
    # Remove config files
    rm -f "$CONFIG_DIR/modular-power.conf"
    ...
    # Remove module disable flags
    rm -f "$CONFIG_DIR/wifi-optimization-disabled"
    rm -f "$CONFIG_DIR/disk-management-disabled"
...
remove_aliases() {
    ...
    if [ -f "$HOME/.bash_aliases" ]; then
        ...
        # Remove new module disable/enable aliases
        sed -i '/alias wifi-off=/d' "$HOME/.bash_aliases"
        sed -i '/alias wifi-on=/d' "$HOME/.bash_aliases"
        sed -i '/alias disk-off=/d' "$HOME/.bash_aliases"
        sed -i '/alias disk-on=/d' "$HOME/.bash_aliases"
        
        # Remove preset shortcuts
        sed -i '/alias ultra-eco=/d' "$HOME/.bash_aliases"
        sed -i '/alias balanced=/d' "$HOME/.bash_aliases"
        sed -i '/alias gaming-max=/d' "$HOME/.bash_aliases"
        sed -i '/alias performance=/d' "$HOME/.bash_aliases"
    fi
...
check_remaining() {
    ...
    if [ "$REMOVE_CONFIGS" = "true" ]; then
        if [ -f "$CONFIG_DIR/modular-power.conf" ] || \
